...
import org.wikitolearn.midtier.course.entity.dto.out.UpdateOrSaveCourseClientDto;
...
public class CourseClient {
  ...
  public EntityList<Course> findAll(MultiValueMap<String, String> params) {
    URI uri = UriComponentsBuilder.fromHttpUrl(baseUrl + "/courses").queryParams(params).build().encode().toUri();

    return client.exchange(uri, HttpMethod.GET, null, new ParameterizedTypeReference<EntityList<Course>>() {
    }).getBody();
  }

  public Course find(String courseId, MultiValueMap<String, String> params) {
    URI uri = UriComponentsBuilder.fromHttpUrl(baseUrl + "/courses/" + courseId).queryParams(params).build().encode()
        .toUri();
  ...
  public EntityList<Course> findByChapterId(String chapterId) throws JsonProcessingException {
    ...
    URI uri = UriComponentsBuilder.fromHttpUrl(baseUrl + "/courses").queryParams(query).build().encode().toUri();
  ...
  public Course save(Course course) throws JsonProcessingException {
    ...
    HttpEntity<UpdateOrSaveCourseClientDto> httpEntity = new HttpEntity<UpdateOrSaveCourseClientDto>(
        modelMapper.map(course, UpdateOrSaveCourseClientDto.class), headers);
